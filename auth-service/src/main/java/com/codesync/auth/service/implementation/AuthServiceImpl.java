...
import com.codesync.auth.kafka.UserLoginEvent;
...
public class AuthServiceImpl implements AuthService {
	...
	public RegisterResponseDto register( RegisterUserDto registerUserDto) {
	    ...
	    java.util.Optional<com.codesync.auth.entity.DeveloperApplicationReceived> devApp = developerApplication.findByEmail(registerUserDto.getEmail());
	...
	public String login(String userName, String password) {
		...
		if(authentication.isAuthenticated()) {
			...
			// ── Publish login notification event to Kafka (fire-and-forget) ──
			try {
				...
				// Never block login due to Kafka issues
				System.out.println("[Kafka] Could not publish login event: " + ex.getMessage());
			}
